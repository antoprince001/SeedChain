...
def createSeed(lotNumber,owner):
...
def certification(lotNumber,SampleSecretCode,CertificateNo, certificateDate):
    """Adds the seed to the blockchain"""
    try:
...
def testing(lotNumber,SamplePassed,SampleSecretCode):
    """Adds the seed to the blockchain"""
    try:
...
def queryTest(lotNumber):
    """Retrieves the test details from the blockchain"""
    try:
        status = contract.functions.queryTest(lotNumber).call()
        if status is True :
